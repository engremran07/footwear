...
| RR-010 | P2-FIXED | ABI-split APK in CI | Release workflow used `--split-per-abi` producing 3 split APKs; the canonical requirement is now the split-per-ABI release artifact set for phone delivery. | Fixed v3.3.0 — `flutter build apk --release --split-per-abi --dart-define=USE_PLAY_INTEGRITY=true` mandated for release artifacts | v2.0.0 |
